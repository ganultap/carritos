	...
	public void run() {
		...
		try {
			...
			if ((tiempoActual >= rangoInicial) && (tiempoActual <= rangoFinal)) {
				Date start = new Date();
				log.info("--- Corriendo proceso automatico Envio de notificacion vehículos reservados no recogidos: "+ start + "----");
				...
				log.info("--- Tarea Automatica de notificacion vehículos reservados no recogidos en espera... "+ new Date() + " ----");
			}
		} catch (Exception e) {
