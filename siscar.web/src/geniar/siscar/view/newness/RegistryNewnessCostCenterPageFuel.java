...
public class RegistryNewnessCostCenterPageFuel extends BaseBean {
	...
	private List<CostCentersFuel> valoresCentroCostosOld;
	private List<CostCentersFuel> valoresCentroCostosNew;
	...
	public void consultarCentroCostos_action(ActionEvent event)
		...
		try {
			...
			VehiclesAssignation assignation = registryNewnessCostCenterService
					.consultarDatosAsignacion(placa.trim().toUpperCase());
			List<Vehicles> listVehicles = new VehiclesDAO()
					.findByVhcPlacaDiplomatica(placa.trim().toUpperCase());
			...
			asignatario = assignation.getRequests().getUsersByRqsUser()
					.getUsrNombre();
			...
			tipoVehiculo = assignation.getVehicles().getVehiclesTypes()
					.getVhtNombre();

			idVehiculo = assignation.getVehicles().getVhcCodigo().toString();
			idAsignacion = assignation.getVhaCodigo().toString();

			valoresCentroCostosOld = SearchNewness
					.ConsultarCentrosDeCostosActualesCombustible(new Long(
							idVehiculo), new Long(idAsignacion));

			if (valoresCentroCostosOld != null
					&& valoresCentroCostosOld.size() > 0)
				setNotEmpty(true);

			setValoresCentroCostosOld(valoresCentroCostosOld);

			FacesUtils.getSession().setAttribute("idVehiculoCostCenter",
					idVehiculo);
			FacesUtils.getSession().setAttribute("idAsignacionCostCenter",
					idAsignacion);
			FacesUtils.getSession().setAttribute("fechaInicial",
					assignation.getVhaFechaInicio());
			FacesUtils.getSession().setAttribute("fechaFinalAsignacion",
					assignation.getVhaFechaTermina());
			activarLabel1();
			desActivarLabel2();
	...
	public void guardarCentroCostos_action(ActionEvent event)
			throws GWorkException {
		Date fechaTermina = (Date) FacesUtils.getSession().getAttribute(
		...
		try {
			if (nombreCentro != null && nombreCentro.trim().length() == 0)
				throw new GWorkException(Util
						.loadErrorMessageValue("NOMBRE.CENTRO"));

			if (nombreCentro != null && nombreCentro.trim().length() != 0)
				esValido = Util
						.validarCadenaCaracteresEspecialesNumLetrasGuion(nombreCentro);

			if (!esValido)
				throw new GWorkException(Util
						.loadErrorMessageValue("NOMBRE.CENTRO.CARACTER"));
			...
			if (!esValido)
				throw new GWorkException(Util
						.loadErrorMessageValue("PORCENTAJE.CENTRO.CARACTER"));
			...
			if (fechaFinal != null)
				FacesUtils.getSession().setAttribute("fechaFinalAsig",
						fechaFinal);

			Date fechaFinalTemp = (Date) FacesUtils.getSession().getAttribute(
					"fechaFinalAsig");

			if (fechaFinal == null && fechaFinalTemp == null)
				throw new GWorkException(Util
						.loadErrorMessageValue("FECHA.CENTRO"));

			Date fechaInicial = (Date) FacesUtils.getSession().getAttribute(
					"fechaInicial");

			/*
			 * Se hace la validación de que la fecha de finalización de los cc
			 * actuales se encuentre dentro de los límites de la asignación del
			 * vehículo, pero para efectos de validar también de que la fecha de
			 * inicio de los nuevos CC se encuentre dentro de esos límites. Es
			 * por eso que se valida la fecha final más 1 día
			 */
			if (fechaFinal != null && fechaInicial != null)
				registryNewnessCostCenterService.validarFechaFinalAsignacion(
						Util.traerFechaMasDias(1, fechaFinal), fechaInicial,
						fechaTermina);

			// String idVehiculo = (String)
			// FacesUtils.getSession().getAttribute(
			// "idVehiculoCostCenter");
			// String idAsignacion = (String) FacesUtils.getSession()
			// .getAttribute("idAsignacionCostCenter");

			// if (fechaFinal != null)
			// registryNewnessCostCenterService
			// .ValidarCentrosDeCostosActuales(new Long(idVehiculo),
			// new Long(idAsignacion), fechaFinalTemp);
			...
			if (valoresCentroCostosNew == null
					|| valoresCentroCostosNew != null
					&& valoresCentroCostosNew.size() == 0) {
				datosCentroCosto(nombreCentro.toUpperCase(), porcentaje);
				desActivarFechas();
				desactivarLimpiar();

			} else if (valoresCentroCostosNew != null
					&& valoresCentroCostosNew.size() > 0) {
				if (validarCampos(valoresCentroCostosNew, Util
						.convertirCadena(porcentaje))) {
					datosCentroCosto(nombreCentro.toUpperCase(), porcentaje);
				}
			}
	...
	public void datosCentroCosto(String centroCosto, String percent)
			throws GWorkException {

		List<CostCentersFuel> valoresCentroCostosNew = getValoresCentroCostosNew();
		if (valoresCentroCostosNew == null)
			valoresCentroCostosNew = new ArrayList<CostCentersFuel>();

		/* Adicionar el nuevo centro de costo ingresado */
		CostCentersFuel objCCV = new CostCentersFuel();
		List<CostsCenters> objCC;
		CostsCenters costCenter = null;

		try {
			objCC = registryNewnessCostCenterService
					.consultarCentroCostos(centroCosto.toUpperCase());
		...
		for (CostCentersFuel ccv : valoresCentroCostosNew) {
			if (ccv.getCostsCenters().getCocNumero().equals(centroCosto))
				existe = true;
		}

		if (!existe) {
			objCCV.setCcfPorcentaje(percent);
	...
	public void guardarListaCC(ActionEvent event) throws GWorkException {
		...
		try {
			...
			String idVehiculo = (String) FacesUtils.getSession().getAttribute(
					"idVehiculoCostCenter");
			String idAsignacion = (String) FacesUtils.getSession()
					.getAttribute("idAsignacionCostCenter");
	...
	public void guardarCentrosCostos(String nombreCentro, String idVehiculo,
			String idAsignacion, String login, Date fechaFinal)
			throws GWorkException {
		if (validarCampos(valoresCentroCostosNew, -1)) {

			java.text.SimpleDateFormat objFormat = new java.text.SimpleDateFormat(
					"yyyy-MM-dd");
			...
			registryNewnessCostCenterService
					.actualizarCentrosDeCostosCombustible(
							valoresCentroCostosOld, login,
							new Long(idVehiculo), fechaInicial, fechaFinal, 0);

			Integer conteo = 0;
			for (CostCentersFuel ccv : valoresCentroCostosNew) {
				registryNewnessCostCenterService
						.guardarCentroCostosCombustible(ccv.getCostsCenters(),
								ccv.getCcfPorcentaje(), idVehiculo,
								idAsignacion, fechaFinal, fechaTermina);
				conteo += 1;
			}

			/*
			 * Si no se ingresa por lo menos un nuevo centro de costo, debe
			 * deshacer los cambios
			 */
			if (conteo <= 0) {
				registryNewnessCostCenterService
						.actualizarCentrosDeCostosCombustible(
								valoresCentroCostosOld, login, new Long(
										idVehiculo), fechaInicial, fechaFinal,
								1);
			} else
				mostrarMensaje(
	...
	public void deleteRow() throws GWorkException {
		List<CostCentersFuel> valoresCCNew = getValoresCentroCostosNew();

		if (valoresCCNew == null || valoresCCNew.size() <= 0)
			throw new GWorkException("lista inicial nula: "
					+ valoresCCNew.size());
	...
	public boolean validarCampos(List<CostCentersFuel> list, int porcentaje)
			throws GWorkException {
		int suma = 0;
		if (list != null) {
			for (CostCentersFuel costsCentersFuel : list) {
				suma += Util.convertirCadena(costsCentersFuel
						.getCcfPorcentaje());
			}
			...
			if (porcentaje == -1) {
				if (suma < 100) {
					mostrarMensaje(Util
							.loadErrorMessageValue("PORCENTAJE.ERROR"), false);
					return false;
				} else
					return true;
			}
	...
	public void validarCamposCentroCosto(ActionEvent event)
			throws GWorkException {
		try {
	...
	public void setRegistryNewnessCostCenterService(
			RegistryNewnessCostCenterService registryNewnessCostCenterService) {
	...
	public List<CostCentersFuel> getValoresCentroCostosOld() {
		return valoresCentroCostosOld;
	}

	public void setValoresCentroCostosOld(
			List<CostCentersFuel> valoresCentroCostosOld) {
		this.valoresCentroCostosOld = valoresCentroCostosOld;
	}

	public List<CostCentersFuel> getValoresCentroCostosNew() {
		return valoresCentroCostosNew;
	}

	public void setValoresCentroCostosNew(
			List<CostCentersFuel> valoresCentroCostosNew) {
