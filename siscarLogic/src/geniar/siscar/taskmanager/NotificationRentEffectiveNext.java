 ...
 */
public class NotificationRentEffectiveNext extends TimerTask{
	
	private Timer timer = null;
	private String repeticionHorasStr;
	private String factorConversionHorasMilisegs;
	private static final Log log = LogFactory.getLog(NotificationRentEffectiveNext.class);
	 ...
	 * @param time
	 */
	public NotificationRentEffectiveNext(Date time){
		try {
			...
			getTimer().scheduleAtFixedRate(new NotificationRentEffectiveNext(),time,repeticionMilisegundos);			
		} 
	...
	public NotificationRentEffectiveNext()
	{
	...
	/**
	 * Metodo abstracto implementado de la super clase TimerTask para la ejecucion de la Tarea
	 * @author Geniar S.A
	 * @FechaCreacion (dd-mm-aaaa) 11-06-2007
	 * @return void
	 */
	public void run()
	{
		...
		{	
			...
			int horaInicialRangoEjecucion = Integer.parseInt(Util.loadParametersValue("HORA_INICIAL_RANGO_EJECUCION_IIII"));
			int minutosInicialRangoEjecucion = Integer.parseInt(Util.loadParametersValue("MINUTOS_INICIAL_RANGO_EJECUCION_IIII"));			
			
			int horaFinalRangoEjecucion = Integer.parseInt(Util.loadParametersValue("HORA_FINAL_RANGO_EJECUCION_IIII"));
			int minutosFinalRangoEjecucion = Integer.parseInt(Util.loadParametersValue("MINUTOS_FINAL_RANGO_EJECUCION_IIII"));
    		...
    		//Pregunta si la hora actual esta dentro del rango de ejecucion de la tarea
	 		if ((tiempoActual >= rangoInicial) && (tiempoActual <= rangoFinal))
	 		{	 			
	 			Date start = new Date();
				log.info("--- Corriendo proceso automatico Envio de notificacion reservas proximas a hacerse efectivas: "+start+"----");
				new NotificationsServiceImpl().notificacionReservasProximasAHacerseEfectivas();
			...
			{
				log.info("--- Tarea Automatica de notificacion reservas proximas a hacerse efectivas en espera... " + new Date() + " ----");
			}
		}
		catch (Exception e) 
		{
			log.error("Error RUN Tarea Automatica de notificacion reservas proximas a hacerse efectivas: " + e.getMessage());
	    }
