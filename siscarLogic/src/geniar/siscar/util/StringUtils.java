...
public abstract class StringUtils {
	...
	// public static final BbcodeOption SHORTEN_URL = new
	// BbcodeOption("SHORTEN_URL");
	// public static final BbcodeOption SANITIZE_URL = new
	// BbcodeOption("SANITIZE_URL");
	// public static final BbcodeOption CONVERT_BARE_URLS = new
	// BbcodeOption("CONVERT_BARE_URLS");
	// public static final BbcodeOption NO_FOLLOW_LINKS = new
	// BbcodeOption("NO_FOLLOW_LINKS");
	 ...
	 * 
	 * @param name
	 *            The string that has to be transformed into a valid class name.
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid URL string.
	 * @return The encoded <code>String</code> object.
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid URL
	 *            parameter string.
	 * @return The encoded <code>String</code> object.
	 ...
	 */
	/*
	 * public static String encodeUrlValue(String source) { if
	 * (!needsUrlEncoding(source)) { return source; } // check if the string is
	 * valid US-ASCII encoding boolean valid = true; CharsetEncoder encoder =
	 * CHARSET_US_ASCII.newEncoder(); try {
	 * encoder.encode(CharBuffer.wrap(source)); } catch
	 * (CharacterCodingException e) { valid = false; }
	 * 
	 * try { // if it is valid US-ASCII, use the regular URL encoding method if
	 * (valid) { return URLEncoder.encode(source, ENCODING_US_ASCII); } //
	 * otherwise, base-64 encode the UTF-8 bytes and mark the string // as being
	 * encoded in a special way else { StringBuilder encoded = new
	 * StringBuilder("%02%02"); String base64 =
	 * Base64.encodeToString(source.getBytes(ENCODING_UTF_8), false); String
	 * base64_urlsafe = replace(base64, "=", "%3D");
	 * encoded.append(base64_urlsafe);
	 * 
	 * return encoded.toString(); } } ///CLOVER:OFF catch
	 * (UnsupportedEncodingException e) { // this should never happen,
	 * ISO-8859-1 is a standard encoding throw new RuntimeException(e); }
	 * ///CLOVER:ON }
	 */
	 ...
	 * 
	 * @param source
	 *            the value that has been encoded for URL usage in a
	 *            RIFE-specific way
	 ...
	 */
	/*
	 * public static String decodeUrlValue(String source) { try { byte[] decoded =
	 * Base64.decode(source.substring(2)); if (null == decoded) { return null; }
	 * else { return new String(decoded, StringUtils.ENCODING_UTF_8); } }
	 * ///CLOVER:OFF catch (UnsupportedEncodingException e) { // this should
	 * never happen, UTF-8 is a standard encoding throw new RuntimeException(e); }
	 * ///CLOVER:ON }
	 */
	 ...
	 * 
	 * @param source
	 *            the value that might have been encoded for URL usage in a
	 *            RIFE-specific way
	...
	public static boolean doesUrlValueNeedDecoding(String source) {
		if (source != null && source.length() > 2 && source.startsWith("\u0002\u0002")) {
			return true;
		}
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid Html
	 *            string.
	 * @return The encoded <code>String</code> object.
	 * @see #encodeClassname(String)
	 * @see #encodeUrl(String)
	 ...
	 * feed existing Html to this method since &amp;, &lt; and &gt; will not be
	 * encoded.
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid Html
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid XML string.
	 * @return The encoded <code>String</code> object.
	 * @see #encodeClassname(String)
	 * @see #encodeUrl(String)
	 * @see #encodeUrlValue(String)
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid sequence of
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid Sql string.
	 * @return The encoded <code>String</code> object.
	 * @see #encodeClassname(String)
	 * @see #encodeUrl(String)
	 * @see #encodeUrlValue(String)
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid LaTeX
	 *            string.
	 * @return The encoded <code>String</code> object.
	 * @see #encodeClassname(String)
	 * @see #encodeUrl(String)
	 ...
	 * 
	 * @param source
	 *            The string that has to be transformed into a valid string,
	 *            using the mappings that are provided through the supplied
	 *            encoding table.
	 * @param encodingTables
	 *            A <code>Map</code> object containing the mappings to
	 *            transform characters into valid entities. The keys of this map
	...
	private static String encode(String source, EncoderFallbackHandler fallbackHandler, Map<Character, String>... encodingTables) {
		...
		for (int i = 0; i < string_to_encode_array.length; i++) {
			...
			if (fallbackHandler != null && last_match < i && fallbackHandler.hasFallback(char_to_encode)) {
				encoded_string = prepareEncodedString(source, encoded_string, i, last_match, string_to_encode_array);
	...
	/**
	 * Transforms a provided <code>String</code> object into a literal that
	 * can be included into a regular expression {@link Pattern} as-is. None of
	 * the regular expression escapes in the string will be functional anymore.
	 * 
	 * @param source
	 *            The string that has to be escaped as a literal
	 * @return The encoded <code>String</code> object.
	 * @see #encodeClassname(String)
	 * @see #encodeUrl(String)
	 * @see #encodeUrlValue(String)
	 ...
	 *            The string whose occurances will we counted.
	 * @param matchCase
	 *            A <code>boolean</code> indicating if the match is going to
	 *            be performed in a case-sensitive manner or not.
	 * @return An <code>int</code> value containing the number of occurances
	 *         of the substring.
	 * @since 1.0
	 */
	...
	/**
	 * Splits a string into different parts, using a seperator string to detect
	 * the seperation boundaries. The seperator will not be included in the
	 * parts array.
	 * 
	 * @param source
	 *            The string that will be split into parts.
	 ...
	 * seperation boundaries in a case-sensitive manner. If a part couldn't be
	 * converted to a <code>byte</code>, it will be omitted from the
	 * resulting array.
	 * 
	 * @param source
	 ...
	 * supplied <code>Collection</code> of <code>String</code> objects
	 * joined by a given seperator.
	 * 
	 * @param path_parts
	 ...
	 * @since 1.0
	 */
	public static String join(ArrayList <String> path_parts, String seperator) {
		if (null == path_parts) {
			return null;
		}
		...
		if (0 == path_parts.size()) {
			return "";
		} else {
			StringBuilder result = new StringBuilder();
			for (String element : path_parts) {
	 ...
	 * 
	 * @param array
	 *            The object array containing the elements to join.
	 * @param seperator
	 *            The seperator used to join the string elements.
	 * @return A new <code>String</code> with the join result.
	 * @since 1.0
	 ...
	 *            The delimiter used to surround the string elements.
	 * @param encodeStrings
	 *            Indicates whether the characters of the string representation
	 *            of the Array values should be encoded.
	 * @return A new <code>String</code> with the join result.
	 * @since 1.0
	 */
	 ...
	 * @param regexps
	 *            The collection of regular expressions against which the match
	 *            will be performed.
	 ...
	 * @param regexp
	 *            The regular expression <code>Pattern</code> against which
	...
	private static String convertUrl(String source, Pattern pattern, boolean shorten, boolean sanitize, boolean no_follow) {
		int max_length = 500; // RifeConfig.Tools.getMaxVisualUrlLength();
		...
		if (found) {
			...
			do {
				...
				if (sanitize) {
					...
					if ((actual_url.indexOf("://") == -1) && (!actual_url.startsWith("/"))) {
	 ...
	 */
	/*
	 * public static String convertBbcode(String source) { if (null == source) {
	 * return null; }
	 * 
	 ...
	 */
	/*
	 * public static String convertBbcode(final String source, BbcodeOption...
	 * options) { if (null == source) { return null; }
	 * 
	 * boolean shorten = false; boolean sanitize = false; boolean convert_bare =
	 * false; boolean no_follow_links = false; if (options != null) { for
	 * (BbcodeOption option : options) { if
	 * (option.equals(StringUtils.SHORTEN_URL)) { shorten = true; } else if
	 * (option.equals(StringUtils.SANITIZE_URL)) { sanitize = true; } else if
	 * (option.equals(StringUtils.CONVERT_BARE_URLS)) { convert_bare = true; }
	 * else if (option.equals(StringUtils.NO_FOLLOW_LINKS)) { no_follow_links =
	 * true; } } }
	 * 
	 * String sourcecopy = source; StringBuilder result = new
	 * StringBuilder(source.length());
	 * 
	 * int startindex; int endIndex; int nextCodeIndex; while (-1 != (startindex =
	 * sourcecopy.indexOf("[code]"))) { // handle parsed String parsed =
	 * sourcecopy.substring(0, startindex); endIndex =
	 * sourcecopy.indexOf("[/code]") + 7; // 7 == the sizeof "[/code]"
	 * nextCodeIndex = sourcecopy.indexOf("[code]", startindex + 6); // 6 == the
	 * sizeof "[code]"
	 * 
	 * if (endIndex < 0) { // not ended... set to end of string endIndex =
	 * sourcecopy.length() - 1; }
	 * 
	 * if (nextCodeIndex < endIndex && nextCodeIndex > 0) { // nested [code]
	 * tags // must end before the next [code] // this will leave a dangling
	 * [/code] but the HTML is valid StringBuilder sourcecopycopy = new
	 * StringBuilder(); sourcecopycopy .append(sourcecopy.substring(0,
	 * nextCodeIndex)) .append("[/code]")
	 * .append(sourcecopy.substring(nextCodeIndex)); sourcecopy =
	 * sourcecopycopy.toString();
	 * 
	 * endIndex = sourcecopy.indexOf("[/code]") + 7; }
	 * 
	 * if (startindex > endIndex) { // dangling [/code] endIndex =
	 * sourcecopy.indexOf("[/code]", endIndex + 7) + 7; // 7 == the sizeof
	 * "[/code]" if (endIndex < 0) { endIndex = sourcecopy.length() - 1; } }
	 * 
	 * String code = sourcecopy.substring(startindex, endIndex);
	 * 
	 * parsed = parseBBCode(parsed, shorten, sanitize, convert_bare,
	 * no_follow_links); // handle raw code = StringUtils.replace(code,
	 * "[code]", "<div class=\"codebody\"><pre>", false); code =
	 * StringUtils.replace(code, "[/code]", "</pre></div>", false);
	 * 
	 * result .append(parsed) .append(code);
	 * 
	 * sourcecopy = sourcecopy.substring(endIndex); }
	 * 
	 * result.append(parseBBCode(sourcecopy, shorten, sanitize, convert_bare,
	 * no_follow_links));
	 * 
	 * return result.toString(); }
	 * 
	 * private static String parseBBCode(String source, boolean shorten, boolean
	 * sanitize, boolean convert_bare, boolean no_follow) { String result =
	 * source;
	 * 
	 * result = StringUtils.replace(result, "[b]", "<b>", false); result =
	 * StringUtils.replace(result, "[/b]", "</b>", false); result =
	 * StringUtils.replace(result, "[u]", "<u>", false); result =
	 * StringUtils.replace(result, "[/u]", "</u>", false); result =
	 * StringUtils.replace(result, "[i]", "<i>", false); result =
	 * StringUtils.replace(result, "[/i]", "</i>", false); result =
	 * StringUtils.replace(result, "[pre]", "<pre>", false); result =
	 * StringUtils.replace(result, "[/pre]", "</pre>", false);
	 * 
	 * String resultCopy = result; String resultLowerCopy =
	 * result.toLowerCase(); StringBuilder buffer = new StringBuilder(); int
	 * startIndex; int endIndex; while (-1 != (startIndex =
	 * resultLowerCopy.indexOf("[*]"))) { int begin =
	 * resultLowerCopy.indexOf("[list]", startIndex + 3); int end =
	 * resultLowerCopy.indexOf("[/list]", startIndex + 3); int next =
	 * resultLowerCopy.indexOf("[*]", startIndex + 3); // 3 == sizeof [*]
	 * 
	 * if (begin == -1) { begin = Integer.MAX_VALUE; }
	 * 
	 * if (end == -1) { end = Integer.MAX_VALUE; }
	 * 
	 * if (next == -1) { next = Integer.MAX_VALUE; }
	 * 
	 * if (next < begin && next < end) { endIndex = next; } else if (begin <
	 * next && begin < end) { endIndex = begin; } else if (end < next && end <
	 * begin) { endIndex = end; } else { endIndex = resultLowerCopy.length(); }
	 * 
	 * buffer .append(resultCopy.substring(0, startIndex)) .append("<li>")
	 * .append(resultCopy.substring(startIndex + 3, endIndex)) // 3 == sizeof
	 * [*] .append("</li>");
	 * 
	 * resultCopy = resultCopy.substring(endIndex); resultLowerCopy =
	 * resultLowerCopy.substring(endIndex); }
	 * buffer.append(resultCopy.substring(0));
	 * 
	 * result = buffer.toString();
	 * 
	 * result = StringUtils.replace(result, "[list]", "<ul>", false); result =
	 * StringUtils.replace(result, "[/list]", "</ul>", false);
	 * 
	 * Matcher color_matcher = BBCODE_COLOR.matcher(result); result =
	 * color_matcher.replaceAll("<font color=\"$1\">"); result =
	 * StringUtils.replace(result, "[/color]", "</font>", false);
	 * 
	 * Matcher size_matcher = BBCODE_SIZE.matcher(result); result =
	 * size_matcher.replaceAll("<font size=\"$1\">"); result =
	 * StringUtils.replace(result, "[/size]", "</font>", false);
	 * 
	 * result = convertUrl(result, BBCODE_URL_SHORT, shorten, sanitize,
	 * no_follow); result = convertUrl(result, BBCODE_URL_LONG, shorten,
	 * sanitize, no_follow);
	 * 
	 * if (convert_bare) { result = convertUrl(result, BBCODE_BAREURL, shorten,
	 * sanitize, no_follow); }
	 * 
	 * Matcher img_matcher = BBCODE_IMG.matcher(result); result =
	 * img_matcher.replaceAll("<div class=\"bbcode_img\"><img src=\"$1\"
	 * border=\"0\" alt=\"\" /></div>");
	 * 
	 * Matcher quote_matcher_long = BBCODE_QUOTE_LONG.matcher(result); result =
	 * quote_matcher_long.replaceAll("<div class=\"quoteaccount\">$1:</div><div
	 * class=\"quotebody\">"); result = StringUtils.replace(result, "[quote]", "<div
	 * class=\"quotebody\">", false); result = StringUtils.replace(result,
	 * "[/quote]", "</div>", false);
	 * 
	 * result = StringUtils.replace(result, "\r\n", "<br />\r"); result =
	 * StringUtils.replace(result, "\n", "<br />\n"); result =
	 * StringUtils.replace(result, "\r", "\r\n"); // remove the BR that could be
	 * added due to code formatting ppl // use to format lists result =
	 * StringUtils.replace(result, "ul><br />\r\n", "ul>\r\n"); result =
	 * StringUtils.replace(result, "ul><br />\n", "ul>\n");
	 * 
	 * return result; }
	 */
	 ...
	 * 
	 * @param document
	 *            a <code>String</code> with the document where the position
	 *            should be looked up in
	 * @param characterIndex
	 *            the index of the character
	 * @return the resulting <code>DocumentPosition</code> instance; or
	 *         <p>
	 *         <code>null</code> if the <code>characterIndex</code> was
	 *         invalid or if the <code>document</code> was null
	 * @since 1.0
	 */
	/*
	 * public static DocumentPosition getDocumentPosition(String document, int
	 * characterIndex) { if (null == document || characterIndex < 0 ||
	 * characterIndex > document.length()) { return null; }
	 * 
	 * int line = 0; int column;
	 * 
	 * String[] linebreaks = new String[] {"\r\n", "\n", "\r"}; int
	 * last_linebreak_index = 0; int next_linebreak_index = document.length();
	 * int match = -1; do { line++;
	 * 
	 * for (String linebreak : linebreaks) { match = document.indexOf(linebreak,
	 * last_linebreak_index); if (match != -1) { if (match >= characterIndex) {
	 * next_linebreak_index = match; match = -1; break; }
	 * 
	 * last_linebreak_index = match + linebreak.length(); break; } } }
	 * while(match != -1);
	 * 
	 * column = characterIndex - last_linebreak_index + 1;
	 * 
	 * return new DocumentPosition(document.substring(last_linebreak_index,
	 * next_linebreak_index), line, column); }
	 */
	...
	/*
	 * public static class BbcodeOption extends EnumClass<String> { protected
	 * BbcodeOption(String identifier) { super(identifier); } }
	 */
}
